...
function fn_cargaGrilla() {
    ...
    $("#jqGrid_lista_A").jqGrid({
        ...
        colNames: ['', 'Nº Contrato', 'Razón Social o Nombre', 'Clasificación del Bien', 'Tipo de Bien', 'Ejecutivo Banca', 'Ejecutivo Leasing', 'Placa Actual', 'Ubicación', 'Moneda', 'Valor del Bien', 'F. Transferencia por OC', 'Descripción del Bien', 'Estado del Contrato', 'Siniestro'],
        colModel: [
                ...
                { name: 'TieneSiniestro', index: 'TieneSiniestro', align: "center", sortable: false, formatter: fn_tieneSiniestro }
        ],
        ...
        ondblClickRow: function(id) {
            parent.fn_blockUI();
            var rowData = $("#jqGrid_lista_A").jqGrid('getRowData', id);
            window.location = "frmSiniestroConsulta.aspx?hddCodContrato=" + rowData.CodSolicitudCredito + "&hddCodBien=" + rowData.SecFinanciamiento;
        }
    });
    ...
    // TieneSiniestro
    //**************************************************
    function fn_tieneSiniestro(cellvalue, options, rowObject) {
        if (fn_util_trim(rowObject.TieneSiniestro) != "0") {
            return "<img src='../../Util/images/ok.gif' alt='" + cellvalue + "' title='El Bien tiene " + rowObject.TieneSiniestro + " siniestros registrados' width='18px' />";
        } else {
            return "."; //<img src='../../Util/images/ok_des.gif' alt='" + cellvalue + "' title='No tiene siniestros registrados' width='18px'/>";
        }
    };
...
// Log			:: 	JRC - 07/11/2012
//****************************************************************
function fn_buscarSiniestro(pblnBusqueda) {
    blnPrimeraBusqueda = pblnBusqueda;
    intPaginaActual = 1;
    fn_realizaBusquedaSiniestro();
}
function fn_realizaBusquedaSiniestro() {
    ...
    try {
        ...
        fn_util_AjaxWM("frmSiniestroListadoConsulta.aspx/ListaSiniestro",
                arrParametros,
                function(jsondata) {
...
// Descripción	::	Abre Consultar Registro
// Log			:: 	WCR - 02/11/2012
//****************************************************************
function fn_abreConsulta() {
    ...
    if (fn_util_trim(codBien) == "") {
        ...
        fn_util_globalRedirect("/Consultas/Siniestro/frmSiniestroConsulta.aspx?hddCodContrato=" + $("#hddCodContrato").val() + "&hddCodBien=" + $("#hddCodBien").val());
    }

}

function fn_SetearTipo(pValue) {
...
//****************************************************************
function fn_Reporte() {
    // window.open("/Reportes/frmRepSiniestro.aspx");
    //    var txtNroContrato = $('#txtNroContrato').val() == undefined ? "" : $('#txtNroContrato').val();
    //    var cmdEstadoContrato = $('#cmdEstadoContrato').val() == "0" ? "" : $('#cmdEstadoContrato').val();
    //    var txtCUCliente = $('#txtCUCliente').val() == undefined ? "" : $('#txtCUCliente').val();
    //    var cmdTipoDoc = $('#cmdTipoDoc').val() == "0" ? "" : $('#cmdTipoDoc').val();
    //    var txtNroDocumento = $('#txtNroDocumento').val() == undefined ? "" : $('#txtNroDocumento').val();
    //    var txtRazonSocial = $('#txtRazonSocial').val() == undefined ? "" : $('#txtRazonSocial').val();
    //    var cmdClasificacion = $('#cmdClasificacion').val() == "0" ? "" : $('#cmdClasificacion').val();
    //    var txtPlaca = $('#txtPlaca').val() == undefined ? "" : $('#txtPlaca').val();
    //    var txtMotor = $('#txtMotor').val() == undefined ? "" : $('#txtMotor').val();
    //    var cmdTipoBien = $('#cmdTipoBien').val() == "0" ? "" : $('#cmdTipoBien').val();
    //    var txtUbicacion = $('#txtUbicacion').val() == undefined ? "" : $('#txtUbicacion').val();

    //    var strParam = '?filtro=' + txtNroContrato + '|' + cmdEstadoContrato + '|' + txtCUCliente + '|' + cmdTipoDoc;
    //    strParam = strParam + '|' + txtNroDocumento + '|' + txtRazonSocial + '|' + cmdClasificacion + '|' + txtPlaca;
    //    strParam = strParam + '|' + txtMotor + '|' + cmdTipoBien + '|' + txtUbicacion;

    //    window.location = "../../Reportes/frmRepSiniestro.aspx" + strParam;
    // fn_util_globalRedirect("/Reportes/frmRepSiniestro.aspx");
    $("#btnGenerar").click();
}