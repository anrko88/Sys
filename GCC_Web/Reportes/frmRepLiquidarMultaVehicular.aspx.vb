...
Imports GCC.LogicWS
Imports System.Data
Partial Class Reportes_frmRepLiquidarMultaVehicular
    Inherits GCCBase
    Private Sub pLlenarTablaRegCompras(ByVal pstrFecha As String, ByVal pstrCodigoImpuesto As String)
        ...
        Try
            lblTitle.Text = "REPORTE LIQUIDAR MULTA VEHICULAR"
            ...
            Dim dtRegCompra As DataTable = GCCUtilitario.DeserializeObject(Of DataTable)(objImpuestoVehicularNTx.ListarMultaVehicularReporteLiquidar(pstrCodigoImpuesto.ToString()))
            ...
            For Each dr As Data.DataRow In dtRegCompra.Rows
                For i As Integer = 0 To strCodImpuesto.Length - 1
                    If dr("SecMulta").ToString = strCodImpuesto(i) Then
                        Dim objCellsCuerpo(11) As TableCell
                        ...
                        objCellsCuerpo(3).Text = GCCUtilitario.CheckStr(dr("NroInfraccion").ToString)

                        objCellsCuerpo(4) = New TableCell
                        objCellsCuerpo(4).Text = GCCUtilitario.CheckStr(dr("TipoBien").ToString)

                        objCellsCuerpo(5) = New TableCell
                        objCellsCuerpo(5).Text = GCCUtilitario.CheckStr(dr("DistritoNombre").ToString)

                        objCellsCuerpo(6) = New TableCell
                        objCellsCuerpo(6).Text = GCCUtilitario.CheckStr(dr("Placa").ToString)

                        objCellsCuerpo(7) = New TableCell
                        objCellsCuerpo(7).Text = GCCUtilitario.CheckStr(dr("Marca").ToString)

                        objCellsCuerpo(8) = New TableCell
                        objCellsCuerpo(8).Text = GCCUtilitario.CheckStr(dr("Modelo").ToString).ToString()
                        'objCellsCuerpo(9).CssClass = "cost"

                        objCellsCuerpo(9) = New TableCell
                        objCellsCuerpo(9).Text = GCCUtilitario.CheckStr(dr("importe").ToString)
                        objCellsCuerpo(9).CssClass = "cost"

                        objCellsCuerpo(10) = New TableCell
                        objCellsCuerpo(10).Text = GCCUtilitario.CheckStr(dr("EstPago").ToString())

                        objCellsCuerpo(11) = New TableCell
                        objCellsCuerpo(11).Text = GCCUtilitario.CheckStr(dr("EstCobro").ToString).ToString()
            ...
            Try
                Dim mNomReporte As String = "rptLiquidarMultaVehicular.xls"
