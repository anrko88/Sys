...
Imports GCC.LogicWS
Imports System.Data
Partial Class Reportes_frmRepLiquidarImpuestoMunicipal
    Inherits GCCBase
    Private Sub pLlenarTablaRegCompras(ByVal pstrFecha As String, ByVal pstrCodigoImpuesto As String)
        ...
        Try
            lblTitle.Text = "REPORTE LIQUIDAR IMPUESTO Y MULTAS MUNICIPALES"
            ...
            Dim dtRegCompra As DataTable = GCCUtilitario.DeserializeObject(Of DataTable)(objImpuestoMunicipalNTx.ListarImpuestoMunicipalReporteLiquidar(pstrCodigoImpuesto.ToString()))
            ...
            For Each dr As Data.DataRow In dtRegCompra.Rows
                For i As Integer = 0 To strCodImpuesto.Length - 1
                    If dr("SecImpuesto").ToString = strCodImpuesto(i) Then
                        Dim objCellsCuerpo(13) As TableCell
                        ...
                        objCellsCuerpo(4).Text = GCCUtilitario.CheckStr(dr("Ubicacion").ToString)
                        ...
                        objCellsCuerpo(8).Text = GCCUtilitario.CheckStr(dr("ImpuestoPredial").ToString)
                        objCellsCuerpo(8).CssClass = "cost"

                        objCellsCuerpo(9) = New TableCell
                        objCellsCuerpo(9).Text = GCCUtilitario.CheckStr(dr("Arbitrio").ToString)
                        objCellsCuerpo(9).CssClass = "cost"

                        objCellsCuerpo(10) = New TableCell
                        objCellsCuerpo(10).Text = GCCUtilitario.CheckStr(dr("Multa").ToString)
                        objCellsCuerpo(10).CssClass = "cost"

                        objCellsCuerpo(11) = New TableCell
                        objCellsCuerpo(11).Text = GCCUtilitario.CheckStr(dr("importe").ToString)
                        objCellsCuerpo(11).CssClass = "cost"

                        objCellsCuerpo(12) = New TableCell
                        objCellsCuerpo(12).Text = GCCUtilitario.CheckStr(dr("EstPago").ToString())

                        objCellsCuerpo(13) = New TableCell
                        objCellsCuerpo(13).Text = GCCUtilitario.CheckStr(dr("EstCobro").ToString).ToString()
