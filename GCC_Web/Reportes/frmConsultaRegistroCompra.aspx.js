...
function fn_registrocompras() {
    ...
    if (fn_util_trim(strFechaInicial) == "") {
		...
		window.open("../Reportes/frmRepRegistroCompra.aspx?p1=" + Fn_util_DateToString(strFechaInicial) + "&p2=" + Fn_util_DateToString(strFechaFinal))    
    }

}

function fn_reportecompras() {

    var strFechaInicial = Fn_util_DateToString($("#txtFechaInicial").val());
    var strFechaFinal = Fn_util_DateToString($("#txtFechaFinal").val());
    ...
    $("#hddFecInicio").val(strFechaInicial)
    $("#hddFecFin").val(strFechaFinal)
