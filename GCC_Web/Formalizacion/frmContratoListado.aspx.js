...
function fn_configuraGrilla() {
    $("#jqGrid_lista_A").jqGrid({
        ...
        colNames: ['Nº Contrato', 'Nº Cotización', 'CU Cliente', 'Razón Social o Nombre', 'Clasificación Bien', 'Estado', 'Fecha', 'Carta', '', '', '', ''],
        colModel: [
        ...
        ondblClickRow: function(id) {
            ...
            var rowData = $("#jqGrid_lista_A").jqGrid('getRowData', id);
            window.location = "frmContratoRegistro.aspx?pTipoTransaccion=EDITAR&hddCodigo=" + rowData.CodSolicitudCredito;
        }
    });
    function fnEl(cellvalue, options, rowObject) {
        if (fn_util_trim(rowObject.CodigoEstadoContrato) == CodigoEstadoContrato.PendienteDeCarta) {
...
function fn_ListadoBienesCarta() {
    ...
    fn_util_AjaxSyncWM("frmContratoRegistro.aspx/ListarBienesDescripcion",
                        arrParametros,
                        function(jsondata) {
...
function fn_abreDetalle() {
    try {
        ...
        if (strId == "" || strId == null || strId == undefined) {
            parent.fn_mdl_mensajeIco("Seleccione un contrato para editarlo.", "util/images/warning.gif", "ERROR EN SELECCION");
        } else {
    ...
    try {
        ...
        if (fn_ValidarDatos()) {
            ...
            var arrParametros = [
                                 ...
                                 "pNotaria",        notaria,
                                 "pKardex", kardex,
                                 "pFields", "CodSolicitudCredito|CodigoCotizacion|CodUnico|NombreSubprestatario|ClasificacionBien|EstadoContrato|FechaSolicitudCredito|ArchivoContratoAdjunto|CodigoEstadoContrato|CorreoContacto|FlagModificado"
                                ];

            fn_util_AjaxWM("frmContratoListado.aspx/BuscarContratos",
                           arrParametros,
                           function(jsondata) {
