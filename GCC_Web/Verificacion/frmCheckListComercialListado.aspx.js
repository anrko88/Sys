...
function fn_cargaGrilla() {
    $("#jqGrid_lista_A").jqGrid({
        ...
        ondblClickRow: function(id) {
            parent.fn_blockUI();
            var rowData = $("#jqGrid_lista_A").jqGrid('getRowData', id);
            window.location = "frmCheckListComercialRegistro.aspx?cc=" + rowData.CODSOLICITUDCREDITO;
        }
    });
...
function fn_ListarContratoCotizacion() {
	...
	if (!blnPrimeraBusqueda) {
		...
		var arrParametros = ["pPageSize", fn_util_getJQGridParam("jqGrid_lista_A", "rowNum"),
			...
			"pZonal", $("#cmbZonalCombo").val(),
			"pFechaInicio", $("#txtFechaIni").val(),
			"pFechaFin", $("#txtFechaFin").val()
		];

		fn_util_AjaxWM("frmCheckListComercialListado.aspx/ListadoContratoCotizacion",
			arrParametros,
			function(jsondata) {
