...
function fn_cargaGrilla() {
    ...
    var updateIdsOfSelectedRows = function(id, isSelected, pIndicador) {
        ...
        if (fn_util_trim(rowData.EstadoPago) != '003') {
            if (!isSelected) {
                ...
                var indexU = $.inArray(id, idsAllId);
                if (indexU < 0) {
                    idsAllId.push(rowData.SecImpuesto);
    ...
    $("#jqGrid_lista_A").jqGrid({
        datatype: function() {
            ...
            fn_ListarImpuestoMunicipalLiquidar();
        },
        jsonReader:
        {
            root: "Items",
        ...
        colNames: ['Nro Contrato', 'CU Cliente', 'Razón Social o Nombre', 'Tipo Bien', 'Ubicación', 'Departamento', 'Provincia', 'Distrito', 'Impuesto Predial', 'Arbitrio', 'Multa', 'Importe', 'Estado Pago', 'Estado Cobro', '', ''],
        colModel: [

                { name: 'CodSolicitudCredito', index: 'CodSolicitudCredito', sortable: true, sorttype: "string", align: "center", defaultValue: "" },
                ...
                { name: 'TipoBien', index: 'TipoBien', sortable: true, sorttype: "string", align: "right", defaultValue: "" },
                { name: 'Ubicacion', index: 'Ubicacion', sortable: true, sorttype: "string", align: "center", defaultValue: "" },
                { name: 'Departamento', index: 'Departamento', sortable: true, sorttype: "string", align: "center", defaultValue: "" },
                { name: 'Provincia', index: 'Provincia', sortable: true, sorttype: "string", align: "center", defaultValue: "" },
                { name: 'Distrito', index: 'Distrito', sortable: true, sorttype: "string", align: "left", defaultValue: "" },
                { name: 'ImpuestoPredial', index: 'ImpuestoPredial', sortable: true, sorttype: "string", align: "right", formatter: Fn_util_ReturnValidDecimal2 },
                { name: 'Arbitrio', index: 'Arbitrio', sortable: true, sorttype: "string", align: "right", formatter: Fn_util_ReturnValidDecimal2 },
                { name: 'Multa', index: 'Multa', sortable: true, sorttype: "string", align: "right", formatter: Fn_util_ReturnValidDecimal2 },
                { name: 'Importe', index: 'Importe', sortable: true, sorttype: "string", align: "right", formatter: Fn_util_ReturnValidDecimal2 },
                { name: 'EstPago', index: 'EstPago', sortable: true, sorttype: "string", align: "center", defaultValue: "" },
                { name: 'EstCobro', index: 'EstCobro', sortable: true, sorttype: "string", align: "center", defaultValue: "" },
        	    { name: 'EstadoPago', index: 'EstadoPago', sortable: true, sorttype: "string", align: "center", hidden: true },
        	    { name: 'SecImpuesto', index: 'SecImpuesto', sortable: true, sorttype: "string", align: "center", hidden: true }
        ],
        width: glb_intWidthPantalla - 55,
        height: '100%',
...
function fn_Valida() {
    ...
    if (strError.toString() != '') {
        ...
        fn_util_AjaxWM("frmImpuestoMultaInmuebleLiquidar.aspx/CheckLote",
                       arrParametros,
                       function(resultado) {
                           //parent.fn_unBlockUI();
                           if (resultado == "-1") {
...
//****************************************************************
// Funcion		:: 	fn_ListarImpuestoMunicipalTodo
// Descripción	::	Realiza Busqueda (Listado)
// Log			:: 	AEP - 01/02/2013
//****************************************************************
function fn_ListarImpuestoMunicipalTodo() {
    //
    //Inicio IBK - AAE - 14/02/2013 - Obtener info del header del lote
    var arrParametros = ["pNroLote", $("#txtNroLote").val()];
    //Inicio IBK JJM
    fn_util_AjaxWM("frmImpuestoMultaInmuebleLiquidar.aspx/ObtenerHeaderLote",
			arrParametros,
			function(jsondata) {
    ...
    fn_util_AjaxWM("frmImpuestoMultaInmuebleLiquidar.aspx/ListaImpuestoMunicipalLiquidarTodo",
			arrParametros,
			function(jsondata) {
			    for (var i = 0; i < jsondata.Items.length; i++) {
			        if (fn_util_trim(jsondata.Items[i].EstadoPago) != '003') {
			            idsAllId.push(jsondata.Items[i].SecImpuesto);
...
// Log			:: 	AEP - 12/11/2012
//****************************************************************
function fn_ListarImpuestoMunicipalLiquidar() {
    if (!blnPrimeraBusqueda) {
        ...
        if (strError.toString() != '') {
            ...
            fn_util_AjaxWM("frmImpuestoMultaInmuebleLiquidar.aspx/ListaImpuestoMunicipalLiquidar",
 			arrParametros,
 			function(jsondata) {
 			    jqGrid_lista_A.addJSONData(jsondata);
 			    if (boolBus == true) {
 			        fn_ListarImpuestoMunicipalTodo();
 			    }
 			    else {
 			        fn_DeshabilitarCheck();
...
function fn_Reporte() {
    ...
    if (idsAllId.length == 0) {
        parent.fn_mdl_mensajeIco("Seleccione al menos un registro para poder generar el reporte.", "util/images/warning.gif", "LIQUIDIDAR MULTA IMPUESTO INMUEBLE");
    } else {
    ...
    if (fn_util_ComparaFecha(strFechaCobro, strFechaActual)) {
        ...
        fn_mdl_confirma('¿Está seguro de liquidar el lote?',
    			function() {
    			    ...
    			    fn_util_AjaxWM("frmImpuestoMultaInmuebleLiquidar.aspx/LiquidarLote",
    					arrParametros,
    					function(resultado) {
    					    parent.fn_unBlockUI();
    					    var arrResultado = resultado.split("|");
    					    if (arrResultado[1] == '0') {
    					        parent.fn_mdl_mensajeOk("Se liquidó correctamente el Lote.", function() { }, "Guardar Liquidar Lote");
    					    }
    					    if (arrResultado[1] == '-1') {
