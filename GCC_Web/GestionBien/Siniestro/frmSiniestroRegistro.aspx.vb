...
Partial Class GestionBien_Siniestro_frmSiniestroRegistro
    Inherits GCCBase
...
#Region "WebMethods"
    ...
    ''' </remarks>
    <WebMethod()> _
    Public Shared Function EliminaSiniestro(ByVal hddCodContrato As String, _
                                             ByVal hddCodBien As String, _
                                             ByVal hddCodSiniestro As String _
                                           ) As String

        ''Variables
        Dim objSiniestroTx As New LSiniestroTx
        ...
        Try
            ...
            With objESiniestro
                ...
                .Secsiniestro = GCCUtilitario.StringToInteger(hddCodSiniestro)
            ...
            'Ejecuta Consulta
            Dim blnResultado As Boolean = objSiniestroTx.EliminarSiniestro(strESiniestro)
    ...
    Protected Sub CargaEditarSiniestroContrato(ByVal pstrCodContrato As String, ByVal strCodBien As String)
        ...
        Try
            ...
            If dtSiniestro.Rows.Count > 0 Then
                ...
                Me.txtNroContrato.Value = dtSiniestro.Rows(0).Item("CodSolicitudCredito").ToString
                Me.txtCUCliente.Value = dtSiniestro.Rows(0).Item("CodUnico").ToString
                Me.txtRazonSocial.Value = dtSiniestro.Rows(0).Item("ClienteRazonSocial").ToString
                Me.txtEstadoContrato.Value = dtSiniestro.Rows(0).Item("EstadoContrato").ToString
                Me.txtClasificacionBien.Value = dtSiniestro.Rows(0).Item("ClasificacionBien").ToString
                Me.txtTipoBien.Value = dtSiniestro.Rows(0).Item("TipoBien").ToString
                Me.txtMoneda.Value = dtSiniestro.Rows(0).Item("NombreMoneda").ToString
                Me.txtValorBien.Value = dtSiniestro.Rows(0).Item("ValorBien").ToString
                Me.txtFecTransferencia.Value = dtSiniestro.Rows(0).Item("FechaTransferencia").ToString

                Me.txtPlaca.Value = dtSiniestro.Rows(0).Item("Placa").ToString
                Me.txtDescripcion.Value = dtSiniestro.Rows(0).Item("Comentario").ToString

                Me.txtUbicacionCab.Value = dtSiniestro.Rows(0).Item("Ubicacion").ToString
                Me.txtEjecutivoBanca.Value = dtSiniestro.Rows(0).Item("DesEjecutivoBanca").ToString
                Me.txtEjecutivoLeasing.Value = dtSiniestro.Rows(0).Item("NombreEjecutivoleasing").ToString
