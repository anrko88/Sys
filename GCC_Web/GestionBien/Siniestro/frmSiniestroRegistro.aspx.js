...
function fn_cancelar() {
    ...
    parent.fn_mdl_confirma(
                    ...
                    function() {                        
                        fn_util_globalRedirect("/GestionBien/Siniestro/frmSiniestroListado.aspx");
                    },
...
function fn_cargaGrilla() {
     ...
     $("#jqGrid_lista_A").jqGrid({
        ...
        colNames: ['','','','Nº Siniestro','Fecha Siniestro','Tipo','Modificación de Contrato','Seguro','Estado del Bien','Nº Poliza','Aplicación del Fondo','F.última actualización','Docs.',''],
        colModel: [
				{ name: 'SecFinanciamiento',index: 'SecFinanciamiento',	hidden: true },
				{ name: 'SecSiniestro',		index: 'SecSiniestro',  hidden: true},
				{ name: 'CodSolCredito',	index: 'CodSolCredito', hidden: true},
    ...
    // TieneSiniestro
    //**************************************************
    function fn_enviaCarta(cellvalue, options, rowObject) {
        return "<img src='../../Util/images/ico_carta_enviado.gif' alt='Enviar Carta' title='Enviar Carta' width='20px' />";
    };
    ...
    function fn_abreDocumentos(cellvalue, options, rowObject) {
		//return ".";
        return "<img src='../../Util/images/ico_docs.gif' alt='Ver Documentos' title='Ver Documentos' width='18px' onclick=\"javascript:fn_GBAbreDocumentos(\'"+rowObject.CodSolCredito+"\',\'"+rowObject.SecFinanciamiento+"\',\'"+rowObject.SecSiniestro+"\',\'"+C_GESTIONBIEN_SINIESTRO+"\');\" style='cursor:pointer;' />";
    };
...
function fn_buscaSiniestro() {
    try {
        ...
        fn_util_AjaxWM("frmSiniestroRegistro.aspx/ListaSiniestro",
                arrParametros,
                function(jsondata) {
...
//****************************************************************
// Funcion		:: 	fn_editarSiniestro
// Descripción	::	Editar Registro
// Log			:: 	JRC - 11/11/2012
//****************************************************************
function fn_editarSiniestro() {
	var codSiniestro = $("#hddCodSiniestro").val();
	if(fn_util_trim(codSiniestro) == ""){
		parent.fn_mdl_mensajeError("Debe seleccionar un Sinestro", function() { }, "VALIDACIÓN");
	}else{
		parent.fn_util_AbreModal("Siniestro :: Editar", "GestionBien/Siniestro/frmSiniestroMnt.aspx?hddCodContrato="+ $("#hddCodContrato").val() +"&hddCodBien="+ $("#hddCodBien").val() +"&hddCodSiniestro="+ $("#hddCodSiniestro").val() + "&hddVer=0", 950, 500, function() { });
	}
}


//****************************************************************
// Funcion		:: 	fn_agregarSiniestro
// Descripción	::	Abre Agregar Registro
// Log			:: 	JRC - 11/11/2012
//****************************************************************
function fn_agregarSiniestro() {
	parent.fn_util_AbreModal("Siniestro :: Nuevo", "GestionBien/Siniestro/frmSiniestroMnt.aspx?hddCodContrato="+ $("#hddCodContrato").val() +"&hddCodBien="+ $("#hddCodBien").val() +"&hddCodUnico="+ $("#hddCodUnico").val(), 950, 500, function() { });
}

//****************************************************************
// Funcion		:: 	fn_verSiniestro
// Descripción	::	Ver Siniestro
// Log			:: 	JRC - 02/11/2012
//****************************************************************
function fn_verSiniestro() {
	var codSiniestro = $("#hddCodSiniestro").val();
	if(fn_util_trim(codSiniestro) == ""){
		parent.fn_mdl_mensajeError("Debe seleccionar un Sinestro", function() { }, "VALIDACIÓN");
	}else{
		parent.fn_util_AbreModal("Siniestro :: Editar", "GestionBien/Siniestro/frmSiniestroMnt.aspx?hddCodContrato="+ $("#hddCodContrato").val() +"&hddCodBien="+ $("#hddCodBien").val() +"&hddCodSiniestro="+ $("#hddCodSiniestro").val() + "&hddVer=1", 950, 500, function() { });
	}
}
