...
function fn_cargaGrilla() {
    ...
    $("#jqGrid_lista_A").jqGrid({
        ...
        colNames: ['Nro. Contrato', 'CU Cliente', 'Razón Social o Nombre', 'Nº Lote', 'Placa Actual', 'Nº Motor', 'Marca', 'Modelo', 'Año Fabricación', 'F. Inscripción Registral', 'Periodo', 'Nº Cuota', 'Moneda', 'Importe', 'Estado Pago', 'Estado Cobro', 'Nº Cheque', '', '', '', '', '', '', ''],
        colModel: [
            ...
            { name: 'RazonSocial', index: 'RazonSocial', width: 110, sorttype: "string", align: "left" },
        	{ name: 'NroLote', index: 'NroLote', width: 40, align: "center", sorttype: "string" },
		    ...
		    { name: 'FechaInscripcion', index: 'FechaInscripcion', width: 70, align: "center", sorttype: "string", formatter: fn_util_ValidaStringFecha },
		    { name: 'Periodo', index: 'Periodo', width: 40, align: "center", sorttype: "string" },
        	{ name: 'NroCuota', index: 'NroCuota', width: 30, align: "center", sorttype: "string" },
        ...
        ondblClickRow: function(id) {
            ...
            //Inicio IBK - AAE - Seocrrige error en la variable
            //strLNroLote = rowData.NroLote;
            strNroLote = rowData.NroLote;
            strPeriodo = rowData.Periodo;
            if (strNroLote != '') {
                fn_util_redirect('frmImpuestoVehicularRegistro.aspx?placa=' + strPlaca + '&tipo=1' + '&codImpuesto=' + strCodImpuesto + '&fecTransferencia=' + strFechaTransferencia + '&cheque=' + strCheque + '&NroLote=' + strNroLote + '&Periodo=' + strPeriodo);
            }
            //Fin IBK
        }
    });
...
function fn_ListarImpuesto() {
    if (!blnPrimeraBusqueda) {
        ...
        fn_util_AjaxWM("frmImpuestoVehicularListado.aspx/ListaImpuestoVehicular",
                   arrParametros,
                   function(jsondata) {
...
function fn_abreEditar() {
    ...
    if (IsNullOrEmpty(id)) {
        ...
        var strFechaTransferencia = rowData.FechaTransferencia;
        var strCheque = rowData.EstadoPago;
        //Solo los qu tienen Lote se pueden modificar.
        if (strNroLote != '') {
            //Inicio IBK - AAE - Se corrige error en nombre de varaib strNroLote
            //fn_util_redirect('frmImpuestoVehicularRegistro.aspx?placa=' + strPlaca + '&tipo=1' + '&codImpuesto=' + strCodImpuesto + '&fecTransferencia=' + strFechaTransferencia + '&cheque=' + strCheque + '&NroLote=' + strLNroLote + '&Periodo=' + strPeriodo);
            fn_util_redirect('frmImpuestoVehicularRegistro.aspx?placa=' + strPlaca + '&tipo=1' + '&codImpuesto=' + strCodImpuesto + '&fecTransferencia=' + strFechaTransferencia + '&cheque=' + strCheque + '&NroLote=' + strNroLote + '&Periodo=' + strPeriodo);
            //Fin IBK
        }
    }
...
function fn_abreLiquidacion() {
    //parent.fn_util_redirect("frmImpuestoVehicularMasivoResumenCarga.aspx");    
    parent.fn_util_AbreModal("Gestión del Bien :: Liquidación", "GestionBien/ImpuestoVehicular/frmImpuestoVehicularLiquidar.aspx",1100, 530, function() { });
}
...
function fn_abreAsignarCheque() {
    //parent.fn_util_redirect("frmImpuestoVehicularMasivoResumenCarga.aspx");
    //Inicio IBK - AAE
    //parent.fn_util_AbreModal("Gestión del Bien :: AsignarCheque", "GestionBien/ImpuestoVehicular/frmImpuestoVehicularAsigCheque.aspx",550, 220, function() { });
    ...
    parent.fn_util_AbreModal("Gestión del Bien :: AsignarCheque", "GestionBien/ImpuestoVehicular/frmImpuestoVehicularAsigCheque.aspx?NroLote=" + strNroLote, 550, 220, function() { });
    //Fin IBK
}
...
function fn_editarLoteParam() {
    ...
    fn_util_AjaxWM("frmImpuestoVehicularListado.aspx/CheckLote",
                   arrParametros,
                   function(resultado) {
                       parent.fn_unBlockUI();
                       if (resultado == "-1") {
                       ...
                       fn_util_redirect('frmImpuestoVehicularRegistro.aspx?tipo=1' + '&NroLote=' + resultado);
                       }
                   },
                   function(resultado) {
                       parent.fn_unBlockUI();
