...
function fn_CargarEditarLote() {
    strLote = $("#txtNroLoteCarga").val();
    strPeriodo = $("#hddPeriodo").val();
    $("#dv_ver").css('display', 'none');

    //Carga Grilla
    if (strLote == "" || strLote == undefined) {
        if ($("#hddFechaTransferencia").val() != '') {
            ...
            parent.fn_util_AbreModal("Gestión del Bien :: Editar", "GestionBien/ImpuestoVehicular/frmImpuestoVehicularRegistroAgregar.aspx?codImp=" +
       	                            $("#hddCodImpuesto").val() + "&placa=" + $("#txtPlaca").val() + "&origen=2" + "&fechaT=" + $("#hddFechaTransferencia").val(), 900, 330, function() { });
            ...
            if ($("#hddTipo").val() == '1') {
                ...
                parent.fn_util_AbreModal("Gestión del Bien :: Editar", "GestionBien/ImpuestoVehicular/frmImpuestoVehicularRegistroAgregar.aspx?codImp=" +
	        	                    $("#hddCodImpuesto").val() + "&placa=" + $("#txtPlaca").val() + "&origen=2" + "&fechaT=" + $("#hddFechaTransferencia").val() + "&cheque=" + $("#hddCheque").val(), 900, 330, function() { });
            } else {
                fn_cargaGrilla();
                $("#dv_ver").css('display', 'none');
            }
...
function fn_cargaGrilla() {
    ...
    $("#jqGrid_lista_A").jqGrid({
        ...
        colNames: ['Nro. Contrato', 'CU Cliente', 'Razón Social o Nombre', 'Tipo de Bien', 'Placa Actual', 'Nº Motor', 'F. Adquisición', 'F. Inscripción Registral', 'Marca', 'Modelo', 'Clase', 'Año Fabricación', 'Estado Bien', 'Estado Contrato', '', '', '', '', ''],
        colModel: [
...
function fn_ListarBienes() {
    if (!blnPrimeraBusqueda) {
        ...
        fn_util_AjaxWM("frmImpuestoVehicularRegistro.aspx/ListaBienesVehicular",
                   arrParametros,
                   function(jsondata) {
...
function fn_cargaGrillaImpuestos() {
    ...
    $("#jqGrid_lista_C").jqGrid({
        ...
        colModel: [
        	...
        	{ name: 'Observaciones', index: 'Observacion', hidden: true },
		    { name: 'CodSolicitudCredito', index: 'CodSolicitudCredito', hidden: true },
            { name: 'SecFinanciamiento', index: 'SecFinanciamiento', hidden: true },
        ...
        ondblClickRow: function(id) {
            ...
            if ((fn_util_trim(estCobro) == "H") || (fn_util_trim(estCobro) == "C") || (fn_util_trim(estCobro) == "I")) {
                cobrado = "1"
            }            
            if ((supervisor == "0") && ((cobrado == "1") || (pagado == "1"))) {                
                parent.fn_util_AbreModal("Gestión del Bien :: Editar", "GestionBien/ImpuestoVehicular/frmImpuestoVehicularRegistroAgregar.aspx?codImp=" +
        	             CodImpuesto + "&placa=" + Placa + "&origen=2" + "&fechaT=" + $("#hddFechaTransferencia").val() + "&cheque=" + $("#hddCheque").val() + "&NroLote=" + strLote + '&Periodo=' + strPeriodo + "&ReadOnly=Y", 900, 330, function() { });
                ...
                if (supervisor == "1") {
                    ...
                    parent.fn_mdl_confirma(Resultado == '' ? "¿Está seguro que desea Editar los Impuestos seleccionados?" : Resultado,
       	        		    function() {
                                parent.fn_blockUI();
       	        		        parent.fn_util_AbreModal("Gestión del Bien :: Editar", "GestionBien/ImpuestoVehicular/frmImpuestoVehicularRegistroAgregar.aspx?codImp=" +
        	             CodImpuesto + "&placa=" + Placa + "&origen=2" + "&fechaT=" + $("#hddFechaTransferencia").val() + "&cheque=" + $("#hddCheque").val() + "&NroLote=" + strLote + '&Periodo=' + strPeriodo + "&ReadOnly=N", 900, 330, function() { });
       	        		    },
...
function fn_ListarImpuestosLote() {
    ...
    if ($("#hidTengoLote").val() != "N") {
        ...
        fn_util_AjaxWM("frmImpuestoVehicularRegistro.aspx/ObtenerInfoLote",
                       arrParametros2,
                       function(jsondata) {
    ...
    fn_util_AjaxWM("frmImpuestoVehicularRegistro.aspx/ListarLoteImpuestoVehicular",
                   arrParametros,
                   function(jsondata) {
...
function fn_abreEliminarImpuesto() {
    ...
    var cobrados = 0;    
    //Fin IBK
    var rowData1 = $("#jqGrid_lista_C").jqGrid('getRowData');
    for (var i = 0; i < rowData1.length; i++) {
        ...
        for (var j = 0; j < vElementosAEditar.length; j++) {
            if (row.SecImpuesto == vElementosAEditar[j]) {
                ...
                if (row.EstadoPago == "Pagado" && row.EstadoCobro == "Pagado") {
                Estado = Estado + 1;
                break;
                }
                if (row.EstadoPago == "Pagado" && row.EstadoCobro == "Pendiente") {
                ...
                //alert(fn_util_trim(row.EstadoPago));
                if (fn_util_trim(row.EstadoPago) == "Pagado") {
                    //alert("Pagado" + row.SecImpuesto);
                    pagados = pagados + 1;

                }
                if ((fn_util_trim(row.CodEstadoCobro) == "H") || (fn_util_trim(row.CodEstadoCobro) == "I") || (fn_util_trim(row.CodEstadoCobro) == "C")) {
                    //alert("Cobrado" + row.SecImpuesto);
                    cobrados = cobrados + 1;
                }
            }            
        } //For
    ...
    if (IsNullOrEmpty(id)) {
        ...
        if (vElementosAEditar != "") {
            ...
            /*if (Estado > 0) {
                parent.fn_mdl_mensajeIco("Uno de los impuestos no puede ser eliminado por su estado cobrado.", "util/images/warning.gif", "EDITAR IMPUESTO");
            } else {
                if (EstadoSup > 0) {
                    Resultado = "Uno de los impuestos no puede ser eliminado por su estado Cobrado.¿Está seguro que desea eliminar los items seleccionados?";
                } else {
                    parent.fn_mdl_confirma(Resultado == '' ? "¿Está seguro que desea eliminar los Impuestos seleccionados?" : Resultado,
           	        		function() {
        ...
        if (vElementosAEditar.length > 1) {
            ...
            if (vElementosAEditar != "") {
                ...
                /*if (Pago == "Pagado") {
                    ...
                    parent.fn_util_AbreModal("Gestión del Bien :: Editar", "GestionBien/ImpuestoVehicular/frmImpuestoVehicularRegistroAgregar.aspx?codImp=" +
	        	         CodImpuesto + "&placa=" + Placa + "&origen=2" + "&fechaT=" + $("#hddFechaTransferencia").val() + "&cheque=" + $("#hddCheque").val() + "&NroLote=" + strLote + '&Periodo=' + strPeriodo, 900, 280, function() { });
                    ...
                    if (supervisor == "1") {
                        ...
                        parent.fn_mdl_confirma(Resultado == '' ? "¿Está seguro que desea Editar los Impuestos seleccionados?" : Resultado,
	        	             ...
	        	             CodImpuesto + "&placa=" + Placa + "&origen=2" + "&fechaT=" + $("#hddFechaTransferencia").val() + "&cheque=" + $("#hddCheque").val() + "&NroLote=" + strLote + '&Periodo=' + strPeriodo + "&ReadOnly=N", 900, 330, function() { });
           	        		    ...
           	        		    "Util/images/question.gif",
           	        		    function() {parent.fn_util_AbreModal("Gestión del Bien :: Editar", "GestionBien/ImpuestoVehicular/frmImpuestoVehicularRegistroAgregar.aspx?codImp=" +
	        	             CodImpuesto + "&placa=" + Placa + "&origen=2" + "&fechaT=" + $("#hddFechaTransferencia").val() + "&cheque=" + $("#hddCheque").val() + "&NroLote=" + strLote + '&Periodo=' + strPeriodo + "&ReadOnly=Y", 900, 330, function() { });
           	        		    },
           	        		    'ELIMINAR IMPUESTO'
           	        	    );  	        	                
...
function fn_abreNuevoImpuesto() {
    ...
    if (IsNullOrEmpty(id)) {
        ...
        //parent.fn_util_AbreModal("Gestión del Bien :: Nuevo", "GestionBien/ImpuestoVehicular/frmImpuestoVehicularRegistroAgregar.aspx?csc=" + CodigoSolicitudCredito + "&placa=" + Placa + "&origen=1" + "&desc=" + Descripcion + "&codBien=" + CodBien + "&CodUnico=" + CodUnico + "&NroLote=" + strLote + '&Periodo=' + strPeriodo , 900, 280, function() { });
        parent.fn_util_AbreModal("Gestión del Bien :: Nuevo", "GestionBien/ImpuestoVehicular/frmImpuestoVehicularRegistroAgregar.aspx?csc=" + CodigoSolicitudCredito + "&placa=" + Placa + "&origen=1" + "&desc=" + Descripcion + "&codBien=" + CodBien + "&CodUnico=" + CodUnico + "&NroLote=" + strLote + '&Periodo=' + strPeriodo + '&codNroLote=' + lote, 900, 330, function() { });
        //Fin IBK
        //			}else{
        //			parent.fn_mdl_mensajeIco("Seleccione un registro para poder editar.", "util/images/warning.gif", "AGREGAR IMPUESTO");
        //            }
...
function fn_Volver() {
    ...
    fn_mdl_confirma('¿Está seguro de volver?',
		function() {
		    ...
		    fn_util_redirect('frmImpuestoVehicularListado.aspx?NroLote=' + $("#txtNroLoteCarga").val());
		    //Fin IBK 
		},
		"../../util/images/question.gif",
		function() {
...
function fn_AsignarLote() {
    ...
    fn_util_AjaxWM("frmImpuestoVehicularRegistro.aspx/ReGenerarLote",
    					arrParametros,
    					function(resultado) {
    					    parent.fn_unBlockUI();
...
//    var strPeriodos = '';
    ...
    if (IsNullOrEmpty(count)) {
        ...
        //                var rowData = $("#jqGrid_lista_C").jqGrid('getRowData', vElementosAEditar[j]);
        //                strContratos = strContratos + rowData.CodSolicitudCredito + ',';
        //            }
        ...
        var arrParametro = ["strCodigoImpuesto", strImpuestos, "strCodigoBien", strCodigoBien, "strPeriodos", strCodigoPeriodo, "strContratos", strContratos];
        ...
        if ($("#hddNroCuotas").val() != "") {
            ...
            fn_mdl_confirma('¿Está seguro de generar lote?',
    			function() {
    			    ...
    			    var arrParametros = ["strCodigoImpuesto", strImpuestos, "strNroLote", strLote];
    ...
    if (IsNullOrEmpty(id)) {
        ...
        if (vElementosAEditar.length > 1) {
            ...
            if (vElementosAEditar != "") {
                ...
                //var CodUnico = rowData.CodUnico;
                parent.fn_util_AbreModal("Gestión del Bien :: Editar", "GestionBien/ImpuestoVehicular/frmImpuestoVehicularRegistroAgregar.aspx?codImp=" +
	        	                    CodImpuesto + "&placa=" + Placa + "&origen=3", 900, 330, function() { });
...
function fn_validaPeriodo() {
    ...
    var lblCheckedResult = Periodo.split(",");
    var pstrPeriodo = 0;
    for (var i = 1; i < lblCheckedResult.length; i++) {
        if (lblCheckedResult[i - 1] != lblCheckedResult[i]) {
            if (lblCheckedResult[i] != "") {
                pstrMunicipalidad = pstrPeriodo + 1;
            }
        }
        if (pstrPeriodo > 0) { break; }
        else { MuniLote = lblCheckedResult[i - 1]; }
    }
    $("#hddPeriodo").val(pstrPeriodo);
}

function fn_ObtieneImpuestos() {
    var rowData = $("#jqGrid_lista_C").jqGrid('getRowData');
    for (var i = 0; i < rowData.length; i++) {
        var row = rowData[i];
        strImpuestos = strImpuestos + row.SecImpuesto + ',';
        strCodigoBien = strCodigoBien + row.SecFinanciamiento + ',';
        strCodigoPeriodo = strCodigoPeriodo + row.Periodo + ',';
        strContratos = strContratos + row.CodSolicitudCredito + ',';
    }

    strImpuestos = strImpuestos.substring(0, strImpuestos.length - 1);
    strCodigoBien = strCodigoBien.substring(0, strCodigoBien.length - 1);
    strCodigoPeriodo = strCodigoPeriodo.substring(0, strCodigoPeriodo.length - 1);
    strContratos = strContratos.substring(0, strContratos.length - 1);
...
function fn_EliminaLote() {
    ...
    fn_util_AjaxSyncWM("frmImpuestoVehicularRegistro.aspx/EliminarLote",
            arrParametros,
                        function(resultado) {
                            parent.fn_unBlockUI();
                            var error = eval("(" + resultado.responseText + ")");
...
function fn_AnularLote() {
    ...
    fn_mdl_confirma('¿Está seguro de Anular el lote?',
		function() {
		    ...
		    var Lote = $("#txtNroLoteCarga").val();
		    var arrParametros = ["pLote", Lote];
		    fn_util_AjaxSyncWM("frmImpuestoVehicularRegistro.aspx/AnularLote",
            arrParametros,
                function(resultado) {
                    parent.fn_unBlockUI();
