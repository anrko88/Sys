...
function fn_inicializaCampos() {
    ...
    $("#txtDescConcepto").attr('disabled', 'disabled');
        ...
        $("#txtMontoCheque").attr('disa bled', 'disabled');
...
function fn_cargaGrilla() {
    ...
    $("#jqGrid_lista_A").jqGrid({
        ...
        colNames: ['Nº Contrato', 'CU Cliente', 'Razón Social o Nombre', 'Tipo de Bien', 'Periodo', 'Nº Cuota', 'Distrito/ Municipalidad', 'Placa', 'Marca', 'Modelo', 'Importe', 'Estado de Pago', 'Estado de Cobro', '', '', ''],
        colModel: [
        	...
        	{ name: 'Periodo', index: 'Periodo', width: 60, align: "center", sorttype: "string" },
		    ...
		    { name: 'Marca', index: 'Marca', width: 40, align: "center", sorttype: "string" },
        	...
        	{ name: '', index: '', width: 1 },
        	{ name: 'SecImpuesto', index: 'SecImpuesto', width: 50, align: "center", sorttype: "string", hidden: true },
...
function fn_CalculoTotal() {
    ...
    //    $("#txtTotal").html(fn_util_AddCommas(fn_util_RedondearDecimales(decTotal, 2)));
    $("#txtTotal").val(fn_util_AddCommas(fn_util_RedondearDecimales(decTotal, 2))); //JJM IBK
...
function fn_Valida() {
    ...
    if (strError.toString() != '') {
        ...
        fn_util_AjaxWM("frmImpuestoVehicularLiquidar.aspx/CheckLote",
                       arrParametros,
                       function(resultado) {
                           //parent.fn_unBlockUI();
                           if (resultado == "-1") {
...
//****************************************************************
// Funcion		:: 	fn_ListarImpuestoTodo
// Descripción	::	Realiza Busqueda (Listado)
// Log			:: 	AEP - 17/11/2012
//****************************************************************
function fn_ListarImpuestoTodo() {
    //
    //Inicio IBK - AAE - 14/02/2013 - Obtener info del header del lote
    var arrParametros = ["pNroLote", $("#txtNroLote").val()];
    //Inicio IBK JJM
    fn_util_AjaxWM("frmImpuestoVehicularLiquidar.aspx/ObtenerHeaderLote",
			arrParametros,
			function(jsondata) {
    ...
    fn_util_AjaxWM("frmImpuestoVehicularLiquidar.aspx/ListaImpuestoVehicularLiquidarTodo",
			arrParametros,
			function(jsondata) {
...
// Log			:: 	AEP - 12/11/2012
//****************************************************************
function fn_ListarImpuestoLiquidar() {
    ...
    if (!blnPrimeraBusqueda) {
        ...
        if (strError.toString() != '') {
            ...
            fn_util_AjaxWM("frmImpuestoVehicularLiquidar.aspx/ListaImpuestoVehicularLiquidar",
			arrParametros,
			function(jsondata) {
    ...
    if (fn_util_ComparaFecha(strFechaCobro, strFechaActual)) {
        ...
        fn_mdl_confirma('¿Está seguro de liquidar el lote?',
    			function() {
    			    ...
    			    fn_util_AjaxWM("frmImpuestoVehicularLiquidar.aspx/LiquidarLote",
    					arrParametros,
    					function(resultado) {
    					    parent.fn_unBlockUI();
