...
Imports GCC.Entity
Imports GCC.LogicWS
Partial Class Administracion_frmComentarioEliminacion
    'Inherits System.Web.UI.Page
    Inherits GCCBase

    Dim objLog As New GCCLog("frmComentarioEliminacion.aspx.vb")
        ...
        If (Array.IndexOf(GCCConstante.DestinoCredito_Inmueble, hidCodClasificacion.Value) <> -1) Then
            ...
            If oESolicitudCreditoEstructuraInmueble IsNot Nothing Then
                With oESolicitudCreditoEstructuraInmueble
                    txtFecha.Value = GCCUtilitario.CheckDateString2(.FechaEliminacion.ToString(), "C")
    ...
    <WebMethod()> _
       Public Shared Function DeshabilitarBienes(ByVal pNumeroContrato As String, _
                                            ByVal pSecFinanciamiento As String, _
                                            ByVal pComentarioBaja As String)

        Try

            Dim objLBien As New LBienTx
            Dim blnResult As Boolean = objLBien.fblnDehabilitarBien(pNumeroContrato, pSecFinanciamiento, pComentarioBaja)
            If blnResult Then
                Return "0"
            Else
                Return "1"
